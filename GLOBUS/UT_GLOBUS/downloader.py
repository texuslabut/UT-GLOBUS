...
[[[x_min, y_max],
  ...
  [x_min+(step_x), y_max]]])
        ...
        processing.merge2(path2 + "/temp/",path2,'GEDI')
        delete = glob.glob(path2 + "/temp/*")
        for zz in delete:
            os.remove(zz)
...
[[[X_min, Y_max],
  ...
  [X_max, Y_max]]])
        ...
        ds_esa.to_netcdf(path2+'/_ESA.nc')
...
[[[x_min, y_max],
  ...
  [x_min+(step_x), y_max]]])
        ...
        processing.merge(path2 + "/temp/",path2,'ESA')
        delete = glob.glob(path2 + "/temp/*")
        for zz in delete:
            os.remove(zz)
...
def alos(X_min, Y_min,  X_max, Y_max, path2, city_name):
    ...
    [[[X_min, Y_max],
      ...
      [X_max, Y_max]]])
        ...
        ds_alos.to_netcdf(path2+'/ALOS.nc')
        print('Done ALOS '+city_name)

    else:
        step_x =  (X_max-X_min)/3
        step_y =  (Y_max-Y_min)/3
        x_min = X_min
        y_max = Y_max
    ...
    [[[x_min, y_max],
      ...
      [x_min+(step_x), y_max]]])
            ...
            y_max = y_max-(step_y)
            x_min = X_min
        processing.merge(path2 + "/temp/",path2,'ALOS')
        delete = glob.glob(path2 + "/temp/*")
        for zz in delete:
            os.remove(zz)
...
[[[x_min, y_max],
  ...
  [x_min+(step_x), y_max]]])
        ...
        processing.merge(path2 + "/temp/",path2,'Pop')
        delete = glob.glob(path2 + "/temp/*")
        for zz in delete:
            os.remove(zz)